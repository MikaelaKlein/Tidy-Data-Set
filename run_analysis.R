tidy_data <- library(plyr); library(dplyr); library(reshape2); library(tidyr)
...
## Tidy up the data
    ...
    arrange(Subject_ID, variable, Activity) %>%
    separate(variable, into = c("Feature", "Measurement", "Axis")) %>%
    select(1, 2, 4, 6) %>%
